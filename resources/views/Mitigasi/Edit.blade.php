...
@section('content')
    <div class="bg-white rounded-lg">
        <div class="p-8">
            ...
            <form action="{{ route('mitigasi.update', ['id' => $mitigasi->id]) }}" method="post">
                @csrf
                    ...
                    <div class="flex-1">
                        <div class="w-full">
                            ...
                            <select class="select select-bordered w-full" name="fungsi_id" id="fungsi_id">
                                <option disabled>-- Silahkan pilih fungsi ---</option>
                                @foreach ($fungsis as $fungsi)
                                <option value="{{ $fungsi->id }} @if (old('fungsi_id', $mitigasi->fungsi_id) == $fungsi->id) ? 'selecterd' : ''
                                @endif">{{ $fungsi->name }}</option>
                                @endforeach
                            </select>
                            @error('fungsi_id')
                        ...
                        <div class="w-full">
                            ...
                            <input type="text" id="kegiatan" name="kegiatan" placeholder="Nama kegiatan"
                                class="input input-bordered w-full" value="{{ $mitigasi->kegiatan }}" />
                            @error('kegiatan')
                                <div class="alert mt-2 alert-error shadow-lg">
                                    <div>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6"
                        ...
                        <div class="w-full">
                            ...
                            <input type="text" id="risk" name="risk" placeholder="Resiko"
                                class="input input-bordered w-full" value="{{ $mitigasi->risk }}" />
                            @error('risk')
                                <div class="alert mt-2 alert-error shadow-lg">
                                    <div>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6"
                        ...
                        <div class="w-full">
                            ...
                            <input type="text" id="sebab" name="sebab" placeholder="Sebab"
                                class="input input-bordered w-full" value="{{ $mitigasi->sebab }}" />
                            @error('sebab')
                                <div class="alert mt-2 alert-error shadow-lg">
                                    <div>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6"
                        ...
                        <div class="w-full">
                            ...
                            <select class="select select-bordered w-full" name="sumber_id" id="sumber_id">
                                <option disabled>-- Silahkan pilih sumber ---</option>
                                @foreach ($sumbers as $sumber)
                                    <option value="{{ $sumber->id }}" @if (old('sumber_id', $mitigasi->sumber_id) == $sumber->id) 
                                        ? 'selected' : ''
                                    @endif>{{ $sumber->name }}</option>
                                @endforeach
                            </select>
                            @error('sumber_id')
                                <div class="alert mt-2 alert-error shadow-lg">
                    ...
                    <div class="flex-1">
                        <div class="w-full">
                            ...
                            <textarea id="solusi" name="solusi" class="h-48 resize-none textarea textarea-bordered w-full"
                                placeholder="Masukan solusi anda disini">{{ $mitigasi->solusi }}</textarea>
                            @error('solusi')
                                <div class="alert mt-2 alert-error shadow-lg">
                                    <div>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6"
